...
const Tooltip = ({ text }: { text: string }) => (
    <div className="group relative ml-2 inline-flex items-center justify-center">
        ...
        <div className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 w-64 -translate-x-1/2 opacity-0 transition-opacity group-hover:opacity-100">
            <div className="rounded bg-gray-100 p-2.5 text-center font-sans text-xs leading-snug font-bold text-gray-900 shadow-xl">
                {text}
            </div>
...
export const CaseManager: React.FC<CaseManagerProps> = ({ category }) => {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case 'Mastered': return 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]';
            case 'Drill': return 'bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.5)]';
            case 'Learning': return 'bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.5)]';
            default: return 'bg-red-600 shadow-[0_0_8px_rgba(220,38,38,0.5)]';
        }
    };
            ...
            <div className="mb-6 shrink-0 space-y-4">
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    ...
                    <div className="relative w-full md:w-64">
                        ...
                        <input
                            type="text"
                            className="block w-full rounded-lg border border-gray-800 bg-gray-950 p-2.5 pl-10 text-sm text-white transition outline-none focus:border-blue-500 focus:ring-blue-500"
                            placeholder={`Search ${category} cases...`}
                            value={searchQuery}
                ...
                <div className="custom-scrollbar flex items-center justify-start gap-2 overflow-x-auto pb-1">
                    {category === 'OBL' && ['Slices', 'CO', 'EO'].map(f => (
                        <button key={f} onClick={() => toggleFilter(f, oblFilters, setOblFilters)} className={`whitespace-nowrap px-3 py-1.5 text-xs font-bold uppercase tracking-wider rounded-lg border transition ${oblFilters.includes(f) ? 'bg-blue-600 border-blue-500 text-white' : 'bg-gray-900 border-gray-700 text-gray-500 hover:text-white'}`}>
                            {f}
                        </button>
                    ))}

                    {category === 'PBL' && ['NP PBL', 'DP PBL', 'CP', 'EP'].map(f => (
                        <button key={f} onClick={() => toggleFilter(f, pblFilters, setPblFilters)} className={`whitespace-nowrap px-3 py-1.5 text-xs font-bold uppercase tracking-wider rounded-lg border transition ${pblFilters.includes(f) ? 'bg-pink-600 border-pink-500 text-white' : 'bg-gray-900 border-gray-700 text-gray-500 hover:text-white'}`}>
                            {f}
                        </button>
                    ))}
                </div>
                ...
                <div className="flex items-center justify-between rounded-lg border border-gray-800 bg-gray-950 p-4">
                    <div>
                        <p className="text-xs font-bold tracking-wider text-gray-500 uppercase">Mastered Cases</p>
                ...
                <div className="custom-scrollbar flex gap-2 overflow-x-auto pb-2">
                    {(['All', 'Unlearned', 'Learning', 'Drill', 'Mastered'] as const).map(f => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg font-bold text-xs uppercase transition ${statusFilter === f ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-950 border border-gray-800 text-gray-400 hover:bg-gray-800 hover:text-white'}`}
                        >
                            {f}
                        </button>
                    ))}
                ...
                {filteredGroups.length === 0 ? (
                    ...
                    filteredGroups.map((group, groupIdx) => (
                        <div key={groupIdx} className="space-y-3">
                            <h3 className="sticky top-0 z-10 border-b border-gray-800 bg-gray-900 pb-2 text-sm font-bold text-blue-400">
                                {group.title}
                            </h3>
                            ...
                            <div className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                                {group.items.map((item: CaseItem, itemIdx: number) => {
                                    ...
                                    return (
                                        <div key={`${groupIdx}-${itemIdx}`} className={`p-3 rounded-lg border transition-all ${currentStatus !== 'Unlearned' ? 'bg-gray-800 border-gray-700' : 'bg-gray-950 border-gray-800/50 opacity-80 hover:opacity-100'}`}>
                                            <div className="mb-2 flex items-start justify-between">
                                                <span className={`text-sm font-bold ${currentStatus === 'Mastered' ? 'text-emerald-400' : 'text-gray-200'} break-all pr-2 leading-tight`}>
                                                    {item.name}
                                                </span>
                                                {item.prob && (
                                                    <span className="rounded border border-purple-800/50 bg-purple-900/30 px-1.5 py-0.5 whitespace-nowrap text-[10px] text-purple-400">
                                                        {item.prob}%
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex flex-col gap-3">
                                                <div className="mb-1 flex w-full gap-1">
                                                    {[
                                                        ...
                                                        <div key={s.status} className="flex flex-1 flex-col items-center gap-1">
                                                            <button
                                                                title={s.status}
                                                                onClick={() => handleFieldChange(item.name, 'status', s.status)}
                                                                className={`w-full h-2 rounded-full transition-all ${currentStatus === s.status ? getStatusColor(s.status) : 'bg-black border border-gray-700 hover:bg-gray-700'}`}
                                                            />
                                                            <span className={`text-[8px] font-bold uppercase tracking-tighter ${currentStatus === s.status ? 'text-gray-200' : 'text-gray-600'}`}>
                                                                {s.label}
                                                            </span>
                                                        </div>
                                                    ))}
                                                ...
                                                {category === 'CSP' && (
                                                    <div className="space-y-2 border-t border-gray-700 pt-2">
                                                        ...
                                                        <div className="flex overflow-hidden rounded border border-gray-700 bg-black">
                                                            <button onClick={() => handleFieldChange(item.name, 'evilness', 'Nice')} className={`flex-1 flex items-center justify-center gap-1 text-[10px] py-1.5 border-r border-gray-700 transition ${evilness === 'Nice' ? 'bg-blue-900/50 text-blue-400 font-bold' : 'text-gray-500 hover:bg-gray-800'}`}>
                                                                <Smile className="h-3 w-3" /> Nice
                                                            </button>
                                                            <button onClick={() => handleFieldChange(item.name, 'evilness', 'Unrated')} className={`flex-1 flex items-center justify-center text-[10px] py-1.5 border-r border-gray-700 transition ${evilness === 'Unrated' ? 'bg-gray-800 text-gray-300 font-bold' : 'text-gray-600 hover:bg-gray-800'}`}>
                                                                -
                                                            </button>
                                                            <button onClick={() => handleFieldChange(item.name, 'evilness', 'Evil')} className={`flex-1 flex items-center justify-center gap-1 text-[10px] py-1.5 transition ${evilness === 'Evil' ? 'bg-orange-900/50 text-orange-500 font-bold' : 'text-gray-500 hover:bg-gray-800'}`}>
